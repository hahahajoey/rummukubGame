...
Test the function to block every invalid movement
  ...
  @less_than_3_tiles
  Scenario: Player end up with 2 tiles in a meld
    Given Game is on
    And Player 1 has "1G, 2G" on hand
    When Player place "1G, 2G"
    And Player end the turn
    Then Player 1 's melds don't has "1G, 2G"

  @run_became_not_valid
  Scenario: Player massed up a valid run
    Given Game is on
    And Player 1 has "1G, 2G, 3G" melds
    When Player place "4S" into Player 1 meld 1
    And Player end the turn
    Then Player 1 's melds don't has "1G, 2G, 3G, 4S"
  ...
  Scenario: Player massed up a valid set
    ...
    And Player end the turn
    Then Player 1 's melds don't has "KH, KC, KS, 4S"
  ...
  Scenario: Player made a meld has too less tiles
    ...
    And Player end the turn
    Then Player 1 's melds don't has "KC, KS", "JH, QH, KH"
  ...
  Scenario: Player placed tiles that are not in his hand
    ...
    And Player end the turn
    Then Player 1 's melds don't has "2G, 4G, 5G"
